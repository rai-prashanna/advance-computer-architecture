 ...
 * 
 * Looks for cache line with the lowest lastUse value. 
 */
static int
get_lru_from_set(avdark_cache_t *self, int index)
{
        int i, min_i = -1;
        long min = LONG_MAX;
        for (i = 0; i < self->assoc; i++)
        {
                if (self->lines[index + i].lastUse < min)
                {
                        min = self->lines[index + i].lastUse;
                        min_i = i;
                }
        }
        return min_i;
...
avdc_access(avdark_cache_t *self, avdc_pa_t pa, avdc_access_type_t type)
{
        ...
        int index = index_from_pa(self, pa) * self->assoc;
        int hit = 0;
        int i;
        count++;

        for (i = 0; i < self->assoc && hit == 0; i++)
                hit = self->lines[index + i].valid && self->lines[index + i].tag == tag;

        if (!hit) {
                i = get_lru_from_set(self, index);
                self->lines[index + i].valid = 1;
                self->lines[index + i].tag = tag;
        }
        self->lines[index + i].lastUse = count;
